...
async function getClanMembers(clanTag, limit) {
    ...
    if (clanTag.startsWith('#')) {
        ...
        var fetchUrl = '';
        if (!limit) {
            fetchUrl = `https://api.clashroyale.com/v1/clans/${clanIdConvertString}/members`;
        } else {
            fetchUrl = `https://api.clashroyale.com/v1/clans/${clanIdConvertString}/members?limit=${limit}`;
        }
